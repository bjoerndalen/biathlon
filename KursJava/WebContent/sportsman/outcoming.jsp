...
<div class="container">
  ...
  <article>
		...
		<table width="100%">
			<%
  Sportsman spm = (Sportsman)session.getAttribute("sportsman");
  Collection <Message> lst_msg = ServiceFactory.DEFAULT.getMessageService().getMessageBySportsman(spm);
  ...
  %>
		...
		<table width="100%">
			...
			<c:if test="${buf.whoname==sportsman.fio}">
			<tr>
				<td>${buf.whomname}</td>
				<td>${buf.whoname}</td>
				<td>${buf.textmsg}</td>
